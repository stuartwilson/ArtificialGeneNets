...
public:
    ...
    vector<double> X, Y, Target;
    vector<int> outputID;

    Greig(string logpath, string nname) : Net(logpath){

        HdfData networkExtra(nname,1);
        networkExtra.read_contained_vals ("x", X);
        networkExtra.read_contained_vals ("y", Y);
        nLocations = X.size();
        networkExtra.read_contained_vals ("inputs", inputID);
        networkExtra.read_contained_vals ("outputs", outputID);
        networkExtra.read_contained_vals ("knockouts", knockoutID);
        ...
        { // Get input values (co-ordinates)
            vector<double> tmp;
            networkExtra.read_contained_vals ("inPatterns", tmp);
        ...
        { // get mappings (each map is a list of indices into the output array)
            vector<int> tmp;
            networkExtra.read_contained_vals ("maps", tmp);
        ...
        { // get output array (target output patterns)
            vector<double> tmp;
            networkExtra.read_contained_vals ("outPatterns", tmp);
    ...
    void setError(void){
        ...
        for(int i=0;i<outputID.size();i++){
            P.J[outputID[i]] = Target[i]-P.X[outputID[i]];
        }
    }
    ...
    void setRandomPattern(){
        ...
        reset();
        Target = Outs[Maps[mapIndex][locationIndex]];
    }
